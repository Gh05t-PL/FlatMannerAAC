...
echo "[STATISTICS][" . date("d-m-Y H:i:s") . "]" . "[WORKING]: \"CHECKING IF SERVER ONLINE\"\n";



if ($socket){
    fclose($socket);
    echo "[STATISTICS][" . date("d-m-Y H:i:s") . "]" . "[WORKING]: \"SERVER ONLINE INSERTING COUNT OF ONLINE PLAYERS\"\n";
    ...
    try {
        ...
        $stmt->execute();
        $stmt = null;
        echo "[STATISTICS][" . date("d-m-Y H:i:s") . "]" . "[OK]: \"ONLINE PLAYERS INSERTED\"\n";
    }catch (Exception $e){
        echo "[STATISTICS][" . date("d-m-Y H:i:s") . "]" . "[EXCEPTION]: \"" . $e->getMessage() . "\"\n";
    }

}else{
    echo "[STATISTICS][" . date("d-m-Y H:i:s") . "]" . "[WORKING]: \"SERVER OFFLINE INSERTING 0\"\n";
