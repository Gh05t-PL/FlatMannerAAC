...
class PowerGamersExtension extends \Twig_Extension
{
    ...
    public function getPowerGamers()
    {
        ...
        $powerGamers = $this->doctrine->getManager()
            ->createNativeQuery("SELECT name,id,(t1.experience - expBefore) as expDiff FROM players t1 INNER JOIN (SELECT player_id, exp as expBefore FROM today_exp) t2 ON t1.id = t2.player_id ORDER BY expDiff DESC LIMIT 5", $rsm)
        ->getResult();
