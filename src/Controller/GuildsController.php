...
class GuildsController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($guilds as $key => $value) {
            $value->members = count($this->getDoctrine()
    ...
    public function guild($id, SessionInterface $session)
    {
        ...
        foreach ($members as $value) {
            if ( $value->getPlayer()->getAccount()->getId() == $session->get('account_id') && $value->getRank()->getLevel() > $loggedRankLevel )
    ...
    public function guildAcceptInvite($id, $playerId, SessionInterface $session)
    {
        ...
        $guild = $this->getDoctrine()->getRepository(Guilds::class)->find($id);
        $guildMember = new GuildMembership();
        $guildRank = $this->getDoctrine()->getRepository(GuildRanks::class)->findOneBy(['guild' => $guild, 'level' => 1]);
    ...
    public function guildInvitationAdd($id, SessionInterface $session, Request $request)
    {
        ...
        foreach ($members as $value) {
            if ( $value->getPlayer()->getAccount()->getId() == $session->get('account_id') && $value->getRank()->getLevel() > $access )
                $access = $value->getRank()->getLevel();
        }
        ...
        if ( $form->isSubmitted() && $form->isValid() ) {
            ...
            $isMember = empty($this->getDoctrine()->getManager()
                ->createNativeQuery("SELECT name FROM players WHERE id IN (SELECT player_id FROM guild_membership WHERE guild_id = {$id}) AND name = '{$player->getName()}'", $rsm)
            ->getArrayResult());
